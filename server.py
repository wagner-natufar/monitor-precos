...
class Produto(BaseModel):
    ...
    kw1: str
    kw2: Optional[str] = ""
    meu_preco: Optional[PrecoCanal] = None
...
async def cadastrar_produto(produto: Produto, payload=Depends(exigir_admin)):
    ...
    if await col_produtos.find_one({"ean": produto.ean}):
        raise HTTPException(status_code=400, detail="Produto já cadastrado com esse EAN")
    doc = produto.dict()
...
async def editar_produto(pid: str, produto: Produto, payload=Depends(exigir_admin)):
    if await col_produtos.find_one({"sku": produto.sku, "_id": {"$ne": ObjectId(pid)}}):
        raise HTTPException(status_code=400, detail="Já existe outro produto com esse SKU")
    if await col_produtos.find_one({"ean": produto.ean, "_id": {"$ne": ObjectId(pid)}}):
        raise HTTPException(status_code=400, detail="Já existe outro produto com esse EAN")
...
@app.get("/historico/{produto_id}/canais")
async def historico_por_canais(produto_id: str, payload=Depends(usuario_aprovado)):
    inicio = (datetime.now() - timedelta(days=7)).isoformat()
    canais = ["Mercado Livre", "Amazon", "Shopee", "Droga Raia"]
    resultado = {}
    for canal in canais:
        registros = [serial(r) async for r in col_historico.find(
            {"produto_id": produto_id, "canal": canal, "data": {"$gte": inicio}}
        ).sort("data", 1)]
        menor_preco = min([r["preco"] for r in registros]) if registros else None
        # Calcula variação média
        variacoes = []
        for i in range(1, len(registros)):
            p_ant = registros[i-1]["preco"]
            p_atual = registros[i]["preco"]
            if p_ant != p_atual:
                variacoes.append((p_atual - p_ant) / p_ant * 100)
        variacao_media = sum(variacoes) / len(variacoes) if variacoes else None
        resultado[canal] = {
            "menor_preco": menor_preco,
            "variacao_media": round(variacao_media, 2) if variacao_media is not None else None,
            "registros": registros
        }
    return resultado
...
@app.get("/stats/{produto_id}")
async def stats_produto(produto_id: str, payload=Depends(usuario_aprovado)):
    total_registros = await col_historico.count_documents({"produto_id": produto_id})
    ultimo = await col_historico.find_one({"produto_id": produto_id}, sort=[("data", -1)])
    return {"total_registros": total_registros, "ultima_coleta": ultimo["data"] if ultimo else None}
...
PYEOF
python3 -m py_compile /mnt/user-data/outputs/monitor-precos/server.py && echo "OK"
